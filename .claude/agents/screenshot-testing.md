...
`MEMORY.md`, форматом `правило → признак (grep/селектор) → severity`. Оттуда
`macket-rules-checker` подхватит проверку, не дожидаясь браузера. Рантайм-только
находки (тайминги, гонки) туда НЕ пиши — им место в обычной секции выше.
