...
`id · таб · тип · тема · рубрика · шапка · автор · заголовок · текст · описание ·
...
- **таб** *(только Activity-лента)* — лейбл таба, в котором показывается фид
  ...
  ни в одном табе.** Тип карточки решает, КАК рисуется (твиттер-ряд vs полная
  карточка), колонка «таб» — В КАКОМ табе он лежит. Сверка по лейблу/`id` таба
...
| Друзья | `druzya` | `photo` / `text` / `video` / `reshare-post` / `friendversary` (twitter-like, §8) |
...
> `.caf.__twitter-like`), а не верстаем заново. Помни про исключение
> `FULL_CARD_TYPES` (см. «Механика»).
