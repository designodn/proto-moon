...
содержимое `main`). Инфраструктура готова: CI довозит на VM `.git`, образ
содержит `git`, а `.env` с токеном CI кладёт на VM сам из секрета.

**Что нужно сделать — один шаг в браузере:**
> SourceCraft → репозиторий → **Settings → Secrets** → добавь секрет
> **`SYNC_GIT_TOKEN`** = GitHub Personal Access Token с правом записи (push) в
> `designodn/proto-moon` (classic PAT, scope `repo`; или fine-grained с
> Contents: Read/Write).

Дальше при каждом деплое CI запишет `/opt/proto-moon/.env` с этим токеном (по
ssh, токен не светится; `.env` = 600, в git не попадает). Готово — кнопка начнёт
пушить. Не задашь секрет → кнопка работает, но без пуша («пуш не прошёл» в
статусе).

Тонкая настройка (по желанию, через тот же `.env` / переменные на VM):
...
Роль робота — `admin` на каталог, осознанно: одним ключом делается всё, включая
правку доступов, и не приходится доразрешать роли под каждую задачу. Плата за
это — ключ из переменной окружения доступен любой сессии в этом окружении с теми
же правами. Если понадобится сузить, команды перевода на `editor` — в разделе
«Управление ключами» ниже.
