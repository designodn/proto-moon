...
пушит их в git — чтобы свежее не терялось при пере-деплое (CI заливает на VM
содержимое `main`, перезатирая `/opt/proto-moon`). Переменные задаются окружению
контейнера (в `docker-compose.yml` или `.env` на VM):
...
Без токена/URL коммит сделается, а пуш — нет (в статусе кнопки «пуш не прошёл»).
